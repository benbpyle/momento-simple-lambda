...
pub(crate) async fn function_handler(event: Request) -> Result<Response<Body>, Error> {
    ...
    let message = json!(
     {
         "message": "Hello, this is an AWS Lambda HTTP request"
     }
    );
...
mod tests {
    ...
    async fn test_http_handler_with_query_string() {
        ...
        query_string_parameters.insert("name".into(), "momento-simple-lambda".into());
        ...
        assert_eq!(
            body_string,
            "Hello momento-simple-lambda, this is an AWS Lambda HTTP request"
        );
    }
}
